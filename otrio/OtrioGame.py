...
class OtrioGame(Game):
    ...
    * Action space: 27 moves (size × row × col) mapped via
    ...
    def __init__(self, n_players: int = 2):
        ...
        self.reserves = np.full((n_players, self.COLORS, self.SIZES),
                                self.PIECES_PER_SIZE, np.int8)

    # ══════════════ Alpha‑Zero required API ══════════════
    def getInitBoard(self):
        """Zero‑filled board and reset reserves/color states."""
        board = np.zeros((self.COLORS, self.SIZES, self.N, self.N),
                         dtype=np.int8)
        self.next_color[:] = 0
    ...
    def getValidMoves(self, board: np.ndarray, player: int) -> np.ndarray:
        ...
        for size in range(self.SIZES):
            if self.reserves[player - 1, c, size] == 0:
                continue
            plane = board[c, size]
